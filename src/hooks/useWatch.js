...
// Build the Fast server iframe URL for a given episode ID
const buildFastServerUrl = (epId) =>
...
const injectFastServer = (apiServers, epId) => {
  ...
  const fastEntries = types.map((type) => ({
    ...
    fastUrl: buildFastServerUrl(epId),
  }));
  ...
  useEffect(() => {
    ...
    const fetchStreamInfo = async () => {
      ...
      try {
        ...
        if (server.isFast) {
          // Update the fastUrl in case episodeId changed since list was built
          const url = buildFastServerUrl(episodeId);
          setStreamUrl(url);
          setStreamInfo(null);
          setIntro(null);
          setOutro(null);
